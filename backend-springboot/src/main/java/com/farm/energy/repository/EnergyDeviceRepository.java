...
/**
 * 答辩注释:
 * - 文件： EnergyDeviceRepository
 * - 层级：仓储层
 * - 职责：基于 Spring Data JPA 提供持久层访问方法。
 * - 此注释仅用于说明，不会改变运行行为。
 */
// 行注释：声明 EnergyDeviceRepository 所在的包路径，体现它在项目中的模块位置。
...
// 行注释：定义 EnergyDeviceRepository 数据访问接口，交给 Spring Data JPA 自动实现。
...
public interface EnergyDeviceRepository extends JpaRepository<EnergyDevice, Long> {
    ...
    List<EnergyDevice> findByStatusOrderByIdAsc(String status);
