...
/**
 * 答辩注释:
 * - 文件： EnergyForecastRepository
 * - 层级：仓储层
 * - 职责：基于 Spring Data JPA 提供持久层访问方法。
 * - 此注释仅用于说明，不会改变运行行为。
 */
// 行注释：声明 EnergyForecastRepository 所在的包路径，体现它在项目中的模块位置。
...
// 行注释：定义 EnergyForecastRepository 数据访问接口，交给 Spring Data JPA 自动实现。
// 类注释：这是 能源预测 模块的数据访问接口，Spring Data JPA 会根据它自动提供增删改查能力。
public interface EnergyForecastRepository extends JpaRepository<EnergyForecast, Long> {
    // 行注释：承接上方业务语句，继续完成当前流程的数据组装或调用。
    List<EnergyForecast> findByForecastDateGreaterThanEqualOrderByCreatedAtDesc(LocalDate fromDate);
